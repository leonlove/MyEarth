...
/** Observer base class for tracking when objects are unRef (there reference count goes to 0) and are being deleted.*/
class OSP_EXPORT Observer
{
    public:
        Observer();
        virtual ~Observer();
...
/** Class used by osg::Ref to track the observers associated with it.*/
class OSP_EXPORT ObserverSet : public OpenSP::Ref
{
    public:

        ObserverSet(const Ref* observedObject);

        Ref* getObserverdObject() { return _observedObject; }
        const Ref* getObserverdObject() const { return _observedObject; }

        /** "Lock" a Ref object i.e., protect it from being deleted
    ...
    protected:
        ...
        ObserverSet(const ObserverSet& rhs): OpenSP::Ref(rhs) {}
