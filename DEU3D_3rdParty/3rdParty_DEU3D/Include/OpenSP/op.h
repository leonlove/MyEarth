...
/** Smart pointer for observed objects, that automatically set pointers to them to null when they are deleted.
  * To use the op<> robustly in multi-threaded applications it is recommend to access the pointer via
  * the lock() method that passes back a sp<> that safely takes a reference to the object to prevent deletion
  ...
  *      possible situations where the op<>'s object may be deleted by one thread whilst being accessed
  *      by another.
  * If you are in any doubt about whether it is safe to access the object safe then use the
  * sp<> op<>.lock() combination. */
template<class T>
class op
{
public:
    typedef T element_type;
    op() : _reference(0), _ptr(0) {}

    /**
     * Create a op from a sp.
     */
    op(const sp<T>& rp)
    {
    ...
    /**
     * Create a op from a raw pointer. For compatibility;
    ...
    /**
     * Assign the op to a sp. The sp will be valid if the
     * Ref object hasn't been deleted and has a ref count > 0.
     */
    bool lock(sp<T>& rptr) const
    {
        if (!_reference)
        {
            rptr = 0;
     ...
     */
    bool operator == (const op& wp) const { return _reference == wp._reference; }
    bool operator != (const op& wp) const { return _reference != wp._reference; }
    bool operator < (const op& wp) const { return _reference < wp._reference; }
    bool operator > (const op& wp) const { return _reference > wp._reference; }
    ...
    // by using the op<>::lock() to get a sp<> that
...
protected:
    ...
    OpenSP::sp<ObserverSet>   _reference;
    T*                          _ptr;
};
