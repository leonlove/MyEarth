...
namespace cmmOSG
{
IntersectionVisitor::IntersectionVisitor(osgUtil::Intersector* intersector, osgUtil::IntersectionVisitor::ReadCallback* readCallback) :
    osgUtil::IntersectionVisitor(intersector, readCallback),
    m_eInterTagetType(ITT_ALL)
{
}


IntersectionVisitor::~IntersectionVisitor(void)
{
}


void IntersectionVisitor::apply(osg::MatrixTransform &matrixtransform)
{
    ID id = matrixtransform.getID();
    if(!id.isValid())
    {
        return osgUtil::IntersectionVisitor::apply(matrixtransform);
    }
    else if(MODEL_ID == id.TileID.m_nType && (ITT_MODEL == m_eInterTagetType || ITT_ALL == m_eInterTagetType))
    {
        return osgUtil::IntersectionVisitor::apply(matrixtransform);
    }
    else if(VIRTUAL_TILE == id.TileID.m_nType && ITT_MODEL == m_eInterTagetType)
    {
        return osgUtil::IntersectionVisitor::apply(matrixtransform);
    }
...
void IntersectionVisitor::apply(osg::Group &group)
{
    ...
    else if(TERRAIN_TILE == id.TileID.m_nType && ITT_TERRAIN == m_eInterTagetType)
    {
        return osgUtil::IntersectionVisitor::apply(group);
    }
    else if((VIRTUAL_TILE == id.TileID.m_nType || TERRAIN_TILE == id.TileID.m_nType) && ITT_ALL == m_eInterTagetType)
    {
        return osgUtil::IntersectionVisitor::apply(group);
    }
