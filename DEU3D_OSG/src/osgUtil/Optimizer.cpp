...
{
    optimize(node, DEFAULT_OPTIMIZATIONS);
...
bool Optimizer::RemoveRedundantNodesVisitor::isOperationPermissible(osg::Node& node)
{
    return node.getNumParents()>0 &&
           !node.getStateSet() &&
           !node.getUserData() &&
           node.getNumUpdateCallback() == 0u &&
           node.getNumEventCallback() == 0u &&
           node.getNumUpdateCallback() == 0u &&
...
void Optimizer::RemoveLoadedProxyNodesVisitor::removeRedundantNodes()
{
        ...
        ++itr)
    {
        ...
        if (group.valid())
        {
            ...
            if (group->getNumUpdateCallback() > 0u) keepData = true;
            if (group->getNumEventCallback() > 0u) keepData = true;
            if (group->getNumCullCallback() > 0u) keepData = true;
