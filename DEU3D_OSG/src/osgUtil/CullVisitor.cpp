...
float CullVisitor::getDistanceToEyePoint(const Vec3& pos) const
{
    return (pos-getEyeLocal()).length();
}

float CullVisitor::getDistanceToViewPoint(const Vec3& pos) const
{
...
float CullVisitor::getDistanceFromEyePoint(const osg::Vec3& pos) const
{
    const Matrix& matrix = *_modelviewStack.back();
    const float dist = distance(pos,matrix);
    return dist;
}
...
void CullVisitor::apply(Geode& node)
{
    ...
    for(unsigned int i=0;i<node.getNumDrawables();++i)
    {
        ...
        const float depth = bb.valid() ? distance(bb.center(),matrix) : 0.0f;
#if _DEBUG
        if (osg::isNaN(depth))
        {
        ...
        for(unsigned int i = 0u; i < numPopStateSetRequired; ++i)
        {
            popStateSet();
        }
