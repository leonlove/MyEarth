...
void Image::write(DataOutputStream* out)
{
    ...
    if(data())
    {
        std::string strExtension = osgDB::getFileExtension(getFileName());
        osg::ref_ptr<osgDB::ReaderWriter> pReaderWriter = osgDB::Registry::instance()->getReaderWriterForExtension(strExtension);
        if(strExtension.empty() || !pReaderWriter.valid())
        {
        ...
        {
            std::ostrstream ss;
            pReaderWriter->writeImage(*this, ss);
            ...
            out->writeInt(size);
            out->writeCharArray(ss.str(), size);
...
void Image::read(DataInputStream* in)
{
    ...
    if(id == IVEIMAGE){
        ...
        if(in->readBool())
        {
            ...
            {
                std::strstream ss(data, dataSize);
                osg::ref_ptr<osg::Image> pTempImage = pReaderWriter->readImage(ss).getImage();
                if(pTempImage.valid())
                {
                    pTempImage->setAllocationMode(osg::Image::NO_DELETE);
                    delete []data;
                    setImage(is, it, ir, internalTextureFormat, pixelFormat,
                        dataType, pTempImage->data(), osg::Image::USE_NEW_DELETE, packing);
                }
                else
                {
                    setImage(is, it, ir, internalTextureFormat, pixelFormat,
