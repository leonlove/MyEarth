...
GraphicsThread::GraphicsThread(const std::string &strName)
    : OperationThread(strName)
{
}   
...
void GraphicsThread::run()
{
    ...
    osg::ref_ptr<GraphicsContext> graphicsContext = dynamic_cast<GraphicsContext*>(_parent.get());
...
BlockAndFlushOperation::BlockAndFlushOperation(const std::string& name):
...
FlushDeletedGLObjectsOperation::FlushDeletedGLObjectsOperation(const std::string& name, double availableTime, bool keep):
    GraphicsOperation(name,keep),
