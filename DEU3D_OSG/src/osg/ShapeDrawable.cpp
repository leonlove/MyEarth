...
class DrawShapeVisitor : public ConstShapeVisitor
{
    ...
    protected:
        ...
        void drawCylinderBody(unsigned int numSegments, float tratio, float radius, float height);
        void drawHalfSphere(unsigned int numSegments, unsigned int numRows, float radius, SphereHalf which, float zOffset = 0.0f);
};


void DrawShapeVisitor::drawCylinderBody(unsigned int numSegments, float tratio, float radius, float height)
{
    const float angleDelta = 2.0f*osg::PI/(float)numSegments;
    const float texCoordDelta = 1.0f/(float)numSegments;

    const float ra_x = height;
    const float ra_y = osg::PI * 2.0 * radius;
    ...
    if (drawFrontFace) {
          ...
          ++bodyi,angle+=angleDelta,texCoord+=texCoordDelta)
      {
          ...
          //gl.TexCoord2f(texCoord * ra_x, ra_y);
          gl.TexCoord2f(ra_x, texCoord * ra_y);
          gl.Vertex3f(c*r,s*r,topz);
  
          //gl.TexCoord2f(texCoord * ra_x, 0.0f);
          gl.TexCoord2f(0.0f, texCoord * ra_y);
      ...
      //gl.TexCoord2f(1.0f * ra_x, ra_y);
      gl.TexCoord2f(ra_x, ra_y);
      gl.Vertex3f(r,0.0f,topz);
  
      //gl.TexCoord2f(1.0f * ra_x, 0.0f);
      gl.TexCoord2f(0.0f, ra_y);
    ...
    if (drawBackFace) {
          ...
          ++bodyi,angle+=angleDelta,texCoord+=texCoordDelta)
      {
          ...
          //gl.TexCoord2f(texCoord * ra_x, 0.0f);
          gl.TexCoord2f(0.0, texCoord * ra_y);
          gl.Vertex3f(c*r,s*r,basez);
...
void DrawShapeVisitor::apply(const Box& box)
{
    ...
    float tratio = (_hints ? _hints->getTextureRatio() : true);
    ...
    if (createBody) {
        ...
        //gl.TexCoord2f(0.0f,tratio);
        gl.TexCoord2f(tratio,tratio);
...
void DrawShapeVisitor::apply(const Cylinder& cylinder)
{
    ...
    unsigned int numSegments = 40;
    float ratio = (_hints ? _hints->getDetailRatio() : 1.0f);
    float tratio = (_hints ? _hints->getTextureRatio() : 1.0f);
    if (ratio > 0.0f && ratio != 1.0f) {
        numSegments = (unsigned int) (numSegments * ratio);
        if (numSegments < MIN_NUM_SEGMENTS)
            numSegments = MIN_NUM_SEGMENTS;
    ...
    // cylinder body
    if (createBody) 
        drawCylinderBody(numSegments, tratio, cylinder.getRadius(), cylinder.getHeight());
    ...
    if (createTop) {
        ...
        gl.Normal3f(0.0f,0.0f,1.0f);
        gl.TexCoord2f(0.5f * tratio,0.5f * tratio);
        ...
        gl.TexCoord2f(1.0f * tratio,0.5f * tratio);
        gl.Vertex3f(r,0.0f,topz);
    ...
    if (createBottom)
    {
            ...
            ++bottomi,angle-=angleDelta,texCoord-=texCoordDelta)
        {
            ...
            gl.TexCoord2f(tratio * (c*0.5f+0.5f),tratio * (s*0.5f+0.5f));
            gl.Vertex3f(c*r,s*r,basez);
        }
...
void DrawShapeVisitor::apply(const Capsule& capsule)
{
    ...
    if (createBody) 
        drawCylinderBody(numSegments, tratio, capsule.getRadius(), capsule.getHeight());
