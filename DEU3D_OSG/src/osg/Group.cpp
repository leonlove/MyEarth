...
bool Group::insertChild( unsigned int index, Node *child )
{
    ...
    if (child)
    {
        ...
        if (child->getNumChildrenRequiringUpdateTraversal() > 0u || child->getNumUpdateCallback() > 0u)
        {
        ...
        // could now require app traversal thanks to the new subgraph,
        // so need to check and update if required.
        if (child->getNumChildrenRequiringEventTraversal() > 0u || child->getNumEventCallback() > 0u)
        {
            setNumChildrenRequiringEventTraversal(getNumChildrenRequiringEventTraversal() + 1u);
        }

        // could now require disabling of culling thanks to the new subgraph,
        // so need to check and update if required.
        if (child->getNumChildrenWithCullingDisabled() > 0u || !child->getCullingActive())
        {
            setNumChildrenWithCullingDisabled(getNumChildrenWithCullingDisabled() + 1u);
        }
...
bool Group::setChild( unsigned  int i, Node* newNode )
{
    if (i<_children.size() && newNode)
    {
        ...
        if (origNode->getNumChildrenRequiringUpdateTraversal() > 0u || origNode->getNumUpdateCallback() > 0u)
        {
            --delta_numChildrenRequiringUpdateTraversal;
        }
        if (newNode->getNumChildrenRequiringUpdateTraversal() > 0u || newNode->getNumUpdateCallback() > 0u)
        {
            ++delta_numChildrenRequiringUpdateTraversal;
        }
        ...
        {
            setNumChildrenRequiringUpdateTraversal(getNumChildrenRequiringUpdateTraversal() + delta_numChildrenRequiringUpdateTraversal);
        }
        ...
        if (origNode->getNumChildrenRequiringEventTraversal() > 0u || origNode->getNumEventCallback() > 0u)
        {
            --delta_numChildrenRequiringEventTraversal;
        }
        if (newNode->getNumChildrenRequiringEventTraversal() > 0u || newNode->getNumEventCallback() > 0u)
        {
            ++delta_numChildrenRequiringEventTraversal;
        }
        ...
        {
            setNumChildrenRequiringEventTraversal(getNumChildrenRequiringEventTraversal() + delta_numChildrenRequiringEventTraversal);
        }

        // could now require disabling of culling thanks to the new subgraph,
        // so need to check and update if required.
        int delta_numChildrenWithCullingDisabled = 0;
        if (origNode->getNumChildrenWithCullingDisabled() > 0u || !origNode->getCullingActive())
        {
            --delta_numChildrenWithCullingDisabled;
        }
        if (newNode->getNumChildrenWithCullingDisabled() > 0u || !newNode->getCullingActive())
        {
            ++delta_numChildrenWithCullingDisabled;
        }
        ...
        {
            setNumChildrenWithCullingDisabled(getNumChildrenWithCullingDisabled() + delta_numChildrenWithCullingDisabled);
        }
