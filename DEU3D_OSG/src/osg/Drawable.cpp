...
void Drawable::compileGLObjects(RenderInfo& renderInfo) const
{
    ...
    if( globj == 0 )
    {
        globj = GLObjectPool::instance()->genDisplayList();
    }
    ...
    //const_cast<Drawable *>(this)->releaseWhiteElephant();
}
...
void Drawable::releaseGLObjects(State* state) const
{
    ...
    if (state)
    {
        ...
        if( globj != 0 )
        {
            GLObjectPool::instance()->deleteDisplayList(contextID,globj);
            globj = 0;
        }    
    }
    else
...
void Drawable::dirtyDisplayList()
{
    ...
    for(i=0;i<_globjList.size();++i)
    {
        if (_globjList[i] != 0)
        {
            GLObjectPool::instance()->deleteDisplayList(i, _globjList[i]);
            _globjList[i] = 0;
        }
    }
#endif
