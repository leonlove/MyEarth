...
float CollectOccludersVisitor::getDistanceToEyePoint(const Vec3& pos) const
{
    return (pos-getEyeLocal()).length();
}

float CollectOccludersVisitor::getDistanceToViewPoint(const Vec3& pos) const
{
    return (pos-getViewPointLocal()).length();
}

float CollectOccludersVisitor::getDistanceFromEyePoint(const Vec3& pos) const
{
    const Matrix& matrix = *_modelviewStack.back();
    const float dist = -(pos[0]*matrix(0,2)+pos[1]*matrix(1,2)+pos[2]*matrix(2,2)+matrix(3,2));
    return dist;
}
