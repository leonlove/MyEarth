...
void TextureCubeMap::apply(State& state) const
{
    ...
    TextureObject& textureObject = const_cast<TextureCubeMap &>(*this).getTextureObject(contextID);
    ...
    if (textureObject.isValid())
    {
        ...
        for (int n=0; n<6; n++)
        {
            ...
            {
                applyTexImage2D_load( state, faceTarget[n], _images[n].get(), _textureWidth, _textureHeight, _numMipmapLevels);
    ...
    else if (imagesValid())
    {
        ...
        textureObject.generate(GL_TEXTURE_CUBE_MAP);
        textureObject.bind();
