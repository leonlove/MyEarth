...
public:
    ...
    void decrementUsageCount()
    {
        ...
        if (_numContexts <= 1 && !_compileContexts.empty())
        {
            _compileContexts.clear();
        }
    }
    
    std::vector<osg::ref_ptr<osg::GraphicsContext> >    _compileContexts;
...
{
    OSG_NOTICE<<"GraphicsContext::getOrCreateCompileContext."<<std::endl;

    {
        OpenThreads::ScopedLock<OpenThreads::Mutex> lock(s_contextIDMapMutex);
        //if (s_contextIDMap[contextID]._compileContext.valid()) return s_contextIDMap[contextID]._compileContext.get();
        ContextIDMap::iterator itorFind = s_contextIDMap.find(contextID);
        if(itorFind != s_contextIDMap.end())
        {
            ContextData &contextData = itorFind->second;
            if(!contextData._compileContexts.empty())
            {
                return contextData._compileContexts.front().get();
            }
        }
    }

    return createCompileContext(contextID);
}


GraphicsContext* GraphicsContext::createCompileContext(unsigned int contextID)
{
    ...
    {    
        OpenThreads::ScopedLock<OpenThreads::Mutex> lock(s_contextIDMapMutex);
        s_contextIDMap[contextID]._compileContexts.push_back(gc);
...
void GraphicsContext::swapBuffers()
{
    ...
    {
        _graphicsThread->add(new SwapBuffersOperation("SwapBuffers Operation in Graphics Context"));
    }
    else
    {
        makeCurrent();
...
void GraphicsContext::createGraphicsThread(const std::string &strThreadName)
{
    if (!_graphicsThread)
    {
        setGraphicsThread(new GraphicsThread(strThreadName));
    }
}
