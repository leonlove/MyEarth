...
//float LOD::_rangeRatio = 1.0f;

//void LOD::setRangeRatio(float flt)
//{
//    //LOD::_rangeRatio = osg::clampAbove(flt, 0.01f);
//}
//
//float LOD::getRangeRatio(void)
//{
//    return 0.0f;//LOD::_rangeRatio;
//}
...
void LOD::traverse(NodeVisitor& nv)
{
    switch(nv.getTraversalMode())
    {
        ...
        case(NodeVisitor::TRAVERSE_ACTIVE_CHILDREN):
        {
            ...
            {
                const MinMaxPair &range = _rangeList[i];
                if (range.first <= required_range && required_range < range.second)
                {
...
bool LOD::addChild(Node *child, float min, float max)
{
    if (Group::addChild(child))
    {
        ...
        _rangeList[_children.size()-1u].first = min;
        _rangeList[_children.size()-1u].second = max;
        return true;
    }
    return false;
}
