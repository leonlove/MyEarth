...
void Texture3D::apply(State& state) const
{
    ...
    TextureObject& textureObject = const_cast<Texture3D &>(*this).getTextureObject(contextID);
    ...
    if (textureObject.isValid())
    {
        ...
        if (_image.get() && getModifiedCount(contextID) != _image->getModifiedCount())
        {
    ...
    else if (_image.valid() && _image->data())
    {
        ...
        textureObject.generate(GL_TEXTURE_3D);
        textureObject.bind();
...
void Texture3D::allocateMipmap(State& state) const
{
    ...
    if (textureObject.isValid() && _textureWidth != 0 && _textureHeight != 0 && _textureDepth != 0)
    {
        const Extensions* extensions = getExtensions(contextID,true);
