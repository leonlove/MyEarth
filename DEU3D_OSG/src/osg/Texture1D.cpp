...
void Texture1D::apply(State& state) const
{
    ...
    TextureObject &textureObject = const_cast<Texture1D &>(*this).getTextureObject(contextID);
    ...
    else if (_image.valid() && _image->data())
    {
        ...
        TextureObject &tex = const_cast<Texture1D &>(*this).getTextureObject(contextID);
        tex.generate(GL_TEXTURE_1D);
        tex.bind();
...
void Texture1D::copyTexImage1D(State& state, int x, int y, int width)
{
    ...
    textureObject.generate(GL_TEXTURE_1D);
    textureObject.bind();
...
void Texture1D::allocateMipmap(State& state) const
{
    ...
    if (textureObject.isValid() && _textureWidth != 0)
    {
        // bind texture
        textureObject.bind();
