...
void ProxyNode::traverse(NodeVisitor& nv)
{
        ...
        _loadingExtReference!=NO_AUTOMATIC_LOADING)
    {
        ...
        {
            nv.getDatabaseRequestHandler()->requestNodeFile(_databasePath+_filenameList[i].first, nv.getNodePath(), nv.getFrameStamp(), _filenameList[i].second, _databaseOptions.get());
        }
    }
    else
    {
...
{
    getDatabaseRequest(getNumChildren()) = NULL;
