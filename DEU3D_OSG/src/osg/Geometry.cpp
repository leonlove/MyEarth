...
void Geometry::compileGLObjects(RenderInfo& renderInfo) const
{
    ...
    if (useVertexArrays)
    {
        ...
        for(BufferObjects::iterator itr = bufferObjects.begin();
            ...
            ++itr)
        {
            ref_ptr<GLBufferObject> glBufferObject = (*itr)->getOrCreateGLBufferObject(contextID);
...
void Geometry::releaseWhiteElephant(void)
{
    if(!getUseDisplayList())    return;

    _normalData.array = NULL;
    _normalData.indices = NULL;

    _colorData.array = NULL;
    _colorData.indices = NULL;

    _secondaryColorData.array = NULL;
    _secondaryColorData.indices = NULL;

    _fogCoordData.array = NULL;
    _fogCoordData.indices = NULL;
