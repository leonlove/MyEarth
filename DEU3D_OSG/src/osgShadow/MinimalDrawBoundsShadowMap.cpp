        ...
        ( ThisClass *st, osgUtil::CullVisitor *cv )
{
    ...
    const unsigned nNumCullCallback = _camera->getNumCullCallback();
    for(unsigned n = 0u; n < nNumCullCallback; n++)
    {
        osg::NodeCallback *callback = _camera->getCullCallback(n);
        if (callback != NULL)
        {
            _camera->addCullCallback( new CameraCullCallback( this, callback ) );
        }
    }
...
//    _boundAnalysisTexture->setShadowComparison(true);
    ...
    _boundAnalysisCamera->addCullCallback( new BaseClass::CameraCullCallback(st) );
