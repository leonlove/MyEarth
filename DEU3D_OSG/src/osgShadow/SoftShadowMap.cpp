...
static const char fragmentSoftShaderSource_withBaseTexture[] =
    ...
    "uniform sampler2D osgShadow_baseTexture1; \n"
    "uniform bool bHasSecondTex;\n"
    "uniform sampler2DShadow osgShadow_shadowTexture; \n"
    "uniform sampler3D osgShadow_jitterTexture; \n"
    ...
    "void main(void) \n"
    "{ \n"
    "  vec4 sceneShadowProj  = gl_TexCoord[2]; \n"
    "  float softFactor = osgShadow_softnessWidth * sceneShadowProj.w; \n"
    "  vec4 smCoord  = sceneShadowProj; \n"
    "  vec3 jitterCoord = vec3( gl_FragCoord.xy / osgShadow_jitteringScale, 0.0 ); \n"
    "  float shadow = 0.0; \n"
    ...
    "  vec4 color; \n"  
    "  vec4 color0 = texture2D( osgShadow_baseTexture, gl_TexCoord[0].xy ); \n"
    "  if(bHasSecondTex) \n"
    "  {\n"
    "      vec4 color1 = texture2D( osgShadow_baseTexture1, gl_TexCoord[1].xy ); \n"
    "      color =gl_Color * mix( color0, color1, color1.w); \n"
    "  }\n"
    "  else\n"
    "  {\n"
    "      color = gl_Color * color0;\n"
    "  }\n"
    // apply shadow, modulo the ambient bias
...
SoftShadowMap::SoftShadowMap():
    _softnessWidth(0.002f),
    _jitteringScale(16.0f),
...
void SoftShadowMap::createUniforms()
{
    ...
    osg::Uniform* baseTextureSampler1 = new osg::Uniform("osgShadow_baseTexture1",(int)_baseTextureUnit + 1);
    _uniformList.push_back(baseTextureSampler1);

    osg::Uniform* baseTextureSampler2 = new osg::Uniform("bHasSecondTex",false);
    _uniformList.push_back(baseTextureSampler2);
...
void SoftShadowMap::setSecondTexture(osg::StateSet *pStateSet, bool bEnable)
{
    if(pStateSet == NULL) return;
    pStateSet->getOrCreateUniform("bHasSecondTex", osg::Uniform::BOOL)->set(bEnable);
}
