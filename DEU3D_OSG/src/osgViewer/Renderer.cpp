...
class GraphicsOperationBlocker
{
public:
    explicit GraphicsOperationBlocker(osg::GraphicsOperation *pOperation)
        : _pOperation(pOperation)
    {
        if(_pOperation.valid())
        {
            _pOperation->blockOperation(true);
        }
    }
    ~GraphicsOperationBlocker(void)
    {
        if(_pOperation.valid())
        {
            _pOperation->blockOperation(false);
        }
    }
    osg::ref_ptr<osg::GraphicsOperation>        _pOperation;
};
...
void Renderer::draw()
{
    ...
    if (sceneView && !_done)
    {
        ...
        if (serializeDraw)
        {
            ...
            GraphicsOperationBlocker blocker(_pCompilingOperation.get());
            sceneView->draw();
        }
        else
        {
