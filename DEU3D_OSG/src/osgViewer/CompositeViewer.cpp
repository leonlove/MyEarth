...
void CompositeViewer::addView(osgViewer::View* view)
{
    ...
    osgViewer::Renderer *pRenderer = dynamic_cast<osgViewer::Renderer *>(view->getCamera()->getRenderer());
    if(pRenderer)
    {
...
unsigned int CompositeViewer::getViewIndex(const osgViewer::View* view) const
{
    for(RefViews::const_iterator itr = _views.begin();
        itr != _views.end();
        ++itr)
    {
        if (*itr == view)
        {
            return itr - _views.begin();
        }
    }
...
void CompositeViewer::realize()
{
    ...
    if (osg::DisplaySettings::instance()->getCompileContextsHint() && _incrementalCompileOperation.valid())
    {
        int numProcessors = OpenThreads::GetNumberOfProcessors();
        int processNum = 0;

        const unsigned nMaxContextID = osg::GraphicsContext::getMaxContextID();
        for(unsigned int i=0u; i<= nMaxContextID; ++i)
        {
            for(unsigned n = 0u; n < 1u; n++)
            {
                osg::GraphicsContext* gc = osg::GraphicsContext::createCompileContext(i);

                gc->createGraphicsThread("CompileThread");
...
void CompositeViewer::eventTraversal()
{
    ...
    osg::Matrixd masterCameraVPW = masterCamera->getViewMatrix() * masterCamera->getProjectionMatrix();
                ...
                ++itr)
            {
                ...
                if (pointerEvent)
                {
                    if (getCameraWithFocus())
                    {
                        ...
                        osg::Matrixd localCameraVPW = getCameraWithFocus()->getViewMatrix() * getCameraWithFocus()->getProjectionMatrix();
                        if (viewport) localCameraVPW *= viewport->computeWindowMatrix();

                        osg::Matrixd matrix( osg::Matrixd::inverse(localCameraVPW) * masterCameraVPW );
            ...
            ++veitr)
        {
            ...
            if (view->getSceneData())
            {
                    ...
                    ++itr)
                {
                    ...
                    if (view->getCamera() && view->getCamera()->getNumEventCallback() > 0u)
                    {
...
void CompositeViewer::updateTraversal()
{
        ...
        ++vitr)
    {
...

        {
            ...
            if (view->getCamera() && view->getCamera()->getNumUpdateCallback() > 0u)
            {
