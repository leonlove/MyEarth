...
bool DepthPartitionSettings::getDepthRange(osg::View& view, unsigned int partition, double& zNear, double& zFar)
{
    switch(_mode)
    {
        ...
        case DYNAMIC_VOLUME:
        {
            const double dblRatio = (_zMid - _zNear) / (_zFar - _zNear);
            const double dblDistance = _currentFarZ - _currentNearZ;
            const double n = _currentNearZ;
            const double m = dblDistance * dblRatio + _currentNearZ;
            const double f = _currentFarZ;
...
void View::assignSceneDataToCameras()
{
    ...
    if (_cameraManipulator.valid())
    {
        _cameraManipulator->setTargetNode(sceneData);
