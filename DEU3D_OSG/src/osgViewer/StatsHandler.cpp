...
StatsHandler::StatsHandler():
    _keyEventTogglesOnScreenStats(osgGA::GUIEventAdapter::KEY_F1),
    _statsType(NO_STATS),
    _initialized(false),
    _threadingModel(ViewerBase::CullDrawThreadPerContext),
...
struct PagerCallback : public virtual osg::NodeCallback
{
    ...
    virtual void operator()(osg::Node* node, osg::NodeVisitor* nv)
    {
        if (_dp.valid())
        {
            ...
            sprintf(tmpText, "%-4d", _dp->getItemCountInPrision());
            _prisonList->setText(tmpText);
        }
    ...
    osg::ref_ptr<osgText::Text> _prisonList;
    double                      _multiplier;
};
    ...
    // viewer stats
    {
            ...
            ++itr)
        {
            ...
            if (dp && dp->isRunning())
            {
                ...
                pos.x() = compileList->getBound().xMax() + 2.0f*_characterSize;

                osg::ref_ptr<osgText::Text> prisonLabel = new osgText::Text;
                _statsGeode->addDrawable( prisonLabel.get() );

                prisonLabel->setColor(colorDP);
                prisonLabel->setFont(_font);
                prisonLabel->setCharacterSize(_characterSize);
                prisonLabel->setPosition(pos);
                prisonLabel->setText("in prison: ");

                pos.x() = prisonLabel->getBound().xMax();

                osg::ref_ptr<osgText::Text> prisonList = new osgText::Text;
                _statsGeode->addDrawable( prisonList.get() );

                prisonList->setColor(colorDP);
                prisonList->setFont(_font);
                prisonList->setCharacterSize(_characterSize);
                prisonList->setPosition(pos);
                prisonList->setText("0");

                pos.x() = prisonList->getBound().xMax() + 2.0f*_characterSize;

                _statsGeode->addCullCallback(new PagerCallback(dp, requestList.get(), compileList.get(), prisonList.get(), 1000.0));
            }
