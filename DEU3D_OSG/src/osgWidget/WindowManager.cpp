...
#else
    _lastPush = NULL;
    WidgetList::iterator itorWidget = widgetList.begin();
    for( ; itorWidget != widgetList.end(); ++itorWidget)
    {
        osg::ref_ptr<Widget> pWidget;
        if(!itorWidget->lock(pWidget))
        {
            continue;
        }

        if(!(pWidget->getEventMask() & ev.type))
        {
            continue;
        }

        ev._window = pWidget->getParent();
        ev._widget = pWidget.get();
        ...
        const bool bCall = pWidget->callMethodAndCallbacks(ev);
        if(bCall)
        {
            _lastPush = pWidget;
            return true;
