...
void Text::computeGlyphRepresentation()
{
    ...
    TextureGlyphQuadMap  textureGlyphQuadMap;
    _lineCount = 0;
    
    if (_text.empty()) 
    {
        OpenThreads::ScopedLock<OpenThreads::Mutex> scope(_mtxTextureGlyphQuadMap);
        _textureGlyphQuadMap.swap(textureGlyphQuadMap);
        ...
        if (itr!=endOfLine_itr)
        {
            ...
            for(;itr!=endOfLine_itr;++itr)
            {
                ...
                if (glyph)
                {
                    ...
                    GlyphQuads& glyphquad = textureGlyphQuadMap[glyph->getTexture()];
...
bool Text::computeAverageGlyphWidthAndHeight(float& avg_width, float& avg_height) const
{
    ...
    for(TextureGlyphQuadMap::const_iterator const_titr=textureGlyphQuadMap.begin();
        const_titr!=textureGlyphQuadMap.end();
        ++const_titr)
    {
...
void Text::computeBackdropPositions(unsigned int contextID) const
{
    ...
    // now apply matrix to the glyphs.
    for(TextureGlyphQuadMap::iterator titr=textureGlyphQuadMap.begin();
        titr!=textureGlyphQuadMap.end();
        ++titr)
    {
        GlyphQuads& glyphquad = titr->second;
        GlyphQuads::Coords2& coords2 = glyphquad._coords;
...
void Text::accept(osg::Drawable::ConstAttributeFunctor& af) const
{
    ...
    for(TextureGlyphQuadMap::const_iterator titr=textureGlyphQuadMap.begin();
        titr!=textureGlyphQuadMap.end();
        ++titr)
    {
