#ifndef DEU_PROXY_NODE_H_B2E31D0B_1353_4340_87AB_C948ADEEE1E5_INCLUDE
#define DEU_PROXY_NODE_H_B2E31D0B_1353_4340_87AB_C948ADEEE1E5_INCLUDE
...
class DEUProxyNode : public osg::Group
{
public:
    DEUProxyNode(void);

    DEUProxyNode(const DEUProxyNode &proxynode, const osg::CopyOp &copyop);

    META_Node(osg, DEUProxyNode);
...
class LODFixer;
class DEUProxyGroup : public osg::Group
{
public:
    DEUProxyGroup(void);
    DEUProxyGroup(const DEUProxyGroup &proxyGroup, const osg::CopyOp &copyop) : m_dblRadius(-1.0){}
    virtual ~DEUProxyGroup(void);
    META_Node(osg, DEUProxyGroup);

public:
    virtual osg::BoundingSphere computeBound(void) const;

public:
    bool            create(const vtm::IVirtualTileManager *pVTManager, const ID &id, const ObjectMatrix &mtxObjectIDs, LODFixer *pLODFixer);
    ...
    DEUProxyNode   *getProxyNode(unsigned nFragmentCount, unsigned nRow, unsigned nCol);
