...
void PipeConnectorBuilder::setType(const std::string &strType)
{
...
{
    CircleFace circle;
    circle.m_dblRadius = port.m_dblRadius * 1.05;
    circle.m_vecNormal = port.m_ptPosition - m_ptCorner;
    const double dblLen = circle.m_vecNormal.normalize();
    circle.m_ptCenter  = m_ptCorner - circle.m_vecNormal * dblLen * 0.5;
    return createHoop(circle, dblLen);
}


osg::Node *PipeConnectorBuilder::createPipeHat(const Port &port) const
{
    osg::Vec3d vecNormal = port.m_ptPosition - m_ptCorner;
    const double dblLen = vecNormal.normalize();

    CircleFace circle1;
    circle1.m_dblRadius = port.m_dblRadius * 1.05;
    circle1.m_vecNormal = vecNormal;
    circle1.m_ptCenter  = m_ptCorner - vecNormal * dblLen * 0.5;
    osg::ref_ptr<osg::Node> pPipeHatNode = createHoop(circle1, dblLen);

    CircleFace circle2;
    circle2.m_dblRadius = port.m_dblRadius * 1.1;
    circle2.m_vecNormal = vecNormal;
    circle2.m_ptCenter  = circle1.m_ptCenter;
    osg::ref_ptr<osg::Node> pHoopNode = createHoop(circle2, circle2.m_dblRadius * 0.5);
...
osg::Node *PipeConnectorBuilder::createWeld(void) const
{
...
osg::Node *PipeConnectorBuilder::buildPipeConnector(void)
{
    ...
    else if(m_strType == "endblock")
    {
        // 盲板
        return createEndBlocker(m_vecJoints[0]);
    }
    else if(m_strType == "pipehat")
    {
        // 管帽
...
osg::Geode *PipeConnectorBuilder::createHoop(const CircleFace &circle, double dblLength) const
{
    CircleFace circle0, circle1;
    circle0.m_dblRadius = circle.m_dblRadius;
    circle0.m_ptCenter  = circle.m_ptCenter;
    circle0.m_vecNormal = circle.m_vecNormal;

    circle1.m_dblRadius = circle.m_dblRadius;
    circle1.m_ptCenter  = circle.m_ptCenter + circle.m_vecNormal * dblLength;
