...
void ModelGroup::coordinateTransform(osg::Vec3d src, osg::Vec3d &dst) const
{
    ...
    //double lon = offset.x() /180.0 * 3.1415926;
    //double lat = offset.y() /180.0 * 3.1415926;
    //osg::Vec3d point;
    //osg::EllipsoidModel::instance()->convertLatLongHeightToXYZ(lat, lon, offset.z(), point.x(), point.y(), point.z());

    //osg::Matrixd matrix ;
    //osg::EllipsoidModel *pEllipsoidModel = osg::EllipsoidModel::instance();
    //const osg::Vec3d vecPlumbLine = pEllipsoidModel->computeLocalPlumbLine(lat, lon);
    //const osg::Quat  qtStand(osg::Vec3d(0.0, 0.0, -1.0), vecPlumbLine);

    //const osg::Vec3d vecEastern = pEllipsoidModel->computeLocalEastern(lat, lon);
    ...
    //matrix.postMultTranslate(point);
    //dst = src * matrix;
    //return;
    ...
    //osg::Matrixd mat4 ;
    //mat4.postMultRotate(qtRotation1);
    //mat4.postMultTranslate(point);
    //src = mat4* src  ;
